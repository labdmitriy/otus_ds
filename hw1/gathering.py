...
from scrappers.quotes_scrapper import QuotesScrapper
...
SCRAPPED_FILE = 'data/vacancies_detail.json'
TABLE_FORMAT_FILE = 'data/result.json'
...
def convert_data_to_table_format():
    ...
    input_storage = JSONStorage(SCRAPPED_FILE)
    quotes_storage = JSONStorage(QUOTES_FILE)
    output_storage = JSONStorage(TABLE_FORMAT_FILE)

    # Your code here
    # transform gathered data from txt file to pandas DataFrame and save as csv

    # Transform vacancies data
    transformer = Transformer()
    transformer.transform(input_storage, output_storage, quotes_storage)


def stats_of_data():
    logger.info("stats")
    storage = JSONStorage(TABLE_FORMAT_FILE)
    ...
    # Get statistics from gathered and transformed data
    analyzer = Analyzer()
    analyzer.analyze(storage)
